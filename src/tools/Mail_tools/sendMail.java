...
public class sendMail {
    ...
    private static final String MAIL_HOST ="mail.smtp.host";//发送邮件的主机
    ...
    private static final String MAIL_WEBSITE ="https://localhost:8080/";//组织主页
     ...
     * @param applicants 编号
     */
    public void sendMessageEmail(String to,String subject, String applicants) throws Exception{
        String content = prettyMessageLayout(applicants);
...
//        message.setText("<h1>hello</h1>");//纯文本
    ...
    public void sendResetEmail(String to,String subject, String code) throws Exception{
        String content = prettyResetLayout("2小时",MAIL_ORGANIZATION,code);
        Properties props = new Properties();//key value:配置参数。真正发送邮件时再配置
        props.setProperty(MAIL_TRANSPORT_PROTOCOL, MAIL_TRANSPORT_PROTOCOL_VALUE);//指定邮件发送的协议，参数是规范规定的
        props.setProperty(MAIL_HOST, MAIL_HOST_VALUE);//指定发件服务器的地址，参数是规范规定的
        props.setProperty("mail.smtp.port","465");
...
//        message.setText("<h1>hello</h1>");//纯文本
     ...
     * @param verifyLink 验证码
     * @param websiteName
     * @return
     */
    private static String prettyResetLayout(String validity,String websiteName,String verifyLink){
        return "<div>您重置密码的验证码是："+verifyLink+"，验证成功后就可以使用"+websiteName+"提供的服务了。</div>\n" +
                "                </td>\n" +
                "            </tr>\n" +
                "            <tr>\n" +
                "                <td colspan=\"2\" style=\"font-size:\n" +
                ...
                "                    <div>该链接的有效期为"+validity+"，如链接超过有效期请输入用户名发送邮件</div>\n" +
                "                </td>\n" +
                "            </tr>\n";
    }
